...
try:
    ...
    WebRtcMode = None
    VideoProcessorBase = object  # fallback to allow class definition
...
def bandpass_filter(sig, low=0.8, high=3.0, fs=30.0):
    ...
    Clamp para nao quebrar quando o FPS cai.
    """
...
with st.sidebar:
    ...
    cam_choice = st.radio(
        "Camera do celular", ["Traseira (environment)", "Frontal (user)"], index=0
    )
    ...
    st.markdown("### Dicas")
    st.markdown(
        "- Coloque o dedo sobre a camera e mantenha firme\n"
        "- Use boa iluminacao\n"
        "- Aguarde alguns segundos para estabilizar o sinal"
    )
    st.markdown("---")
    if st.button("Nova medicao"):
...
<div class="main-header">
  <h1>PPG Cardiaco em Tempo Real</h1>
  <p>Medicao via camera do celular (WebRTC) + grafico ao vivo</p>
</div>
""",
    unsafe_allow_html=True,
)
...
# ===================== WebRTC config =====================
# Usar dict simples para compatibilidade entre versões do streamlit-webrtc
...
class PPGProcessor(VideoProcessorBase):
    ...
    def recv(self, frame: av.VideoFrame) -> av.VideoFrame:
        ...
        # ROI central quadrada ~50% do menor lado
        ...
        # quando janela cheia e dedo presente, processa
        ...
        cv2.putText(
            img, f"FPS: {s.fps:.1f}", (w - 180, 40), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 0, 0), 2
        )
...
if not WEBRTC_AVAILABLE:
    st.error(
        "Dependencias do WebRTC ausentes. Instale: "
...
with col_right:
    ...
    if st.button("Salvar"):
        if s.bpm_fixed:
            ...
            st.success(f"Medicao salva: {row['Nome']} - {row['BPM']} BPM")
        else:
            st.warning(
                "Ainda nao ha BPM fixado. Aguarde estabilizar (precisa de ~3 amostras)."
            )
    ...
    if not st.session_state.history.empty:
        ...
        st.info("Nenhuma medicao salva ainda.")
...
with col_left:
    ...
    if s.last_signal_ready:
        ...
        st.markdown(
            '<div class="status">Posicione o dedo sobre a camera e aguarde o sinal estabilizar.</div>',
            unsafe_allow_html=True,
        )
...
st.markdown("---")
st.caption(
    "Dica: Em redes locais, abrir o app via `http://IP_DO_SEU_PC:8501` funciona no celular. "
    "Para fora da rede/local, use HTTPS (o navegador exige HTTPS para liberar a camera)."
)
